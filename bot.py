...
MODEL_PRETTY_NAMES = {
    ...
    "google/gemini-2.0-flash": "Gemini 2.0 Flash ⚡️ (Мультимод.)",
    "google/gemini-flash-1.5": "Gemini 1.5 Flash ⚡️ (Мультимод.)",
    "google/gemini-pro-1.5": "Gemini 1.5 Pro 💫 (Мультимод.)",
    "meta-llama/llama-3.3-70b-instruct": "Llama 3.3 70B 🦙",
    "anthropic/claude-3.5-sonnet": "Claude 3.5 Sonnet 🎭 (Мультимод.)",
    "anthropic/claude-3-haiku": "Claude 3 Haiku ⚡️ (Мультимод.)",
    "openai/gpt-4o": "ChatGPT-4o 🟢 (Мультимод.)",
    "openai/gpt-4o-mini": "ChatGPT-4o Mini 🟢 (Мультимод.)",
...
def is_multimodal_model(model_id: str) -> bool:
    ...
    return any(k in mid for k in ["gemini", "gpt-4o", "claude-3", "sonnet", "haiku", "vision", "qwen-vl"])
...
GREETING_WORDS = {
    "привет", "здравствуйте", "здравствуй", "хай", "хеллоу", 
    "ку", "добрый день", "добрый вечер", "доброе утро", 
    "салам", "шо ты", "как дела", "чо как", "йоу", "hello", "hi", "hey"
}
...
DIRECT_PROMPTS = {
    "ru": (
        ...
        "Твоя текущая языковая модель: {model_name}. Если тебя спрашивают, какая ты модель — честно называй именно {model_name}.\n\n"
        "ТВОЯ РОЛЬ И ВОЗМОЖНОСТИ:\n"
        "1. В этом личном чате: универсальный помощник. Отвечай на любые вопросы, пиши тексты, код, помогай в проектах.\n"
        "2. В бизнес-чатах: умный авто-секретарь для Telegram Business.\n\n"
        "КОМАНДЫ ДЛЯ ПОЛЬЗОВАТЕЛЯ:\n"
        "• /new — начать новый диалог (очистить контекст текущей беседы)\n"
        "• /memory — посмотреть досье и факты о себе\n"
        "• /forget — стереть накопленную память\n"
        "• /settings — панель моделей и пресетов секретаря"
    ),
    "en": (
        "You are 'Ai Assistant', a smart personal Telegram bot with long-term memory.\n"
        "Your current model engine is: {model_name}. Always state that you run on {model_name} when asked.\n\n"
        "COMMANDS:\n"
        "• /new — Fresh topic context\n"
        "• /memory — View profile memory\n"
        "• /forget — Wipe facts\n"
...
PRESETS_RU = {
    "strict": (
        ...
        "КРИТИЧЕСКИЕ ПРАВИЛА (ПО СТРОГОМУ ПРИОРИТЕТУ):\n"
        "1. ДЕЛОВЫЕ ЗАПРОСЫ И РЕКЛАМА (ВЫСШИЙ ПРИОРИТЕТ):\n"
        "   • Если пользователь пишет о рекламе, покупке, заказе, цене или сотрудничестве — ВСЕГДА подтверждай приём заявки, даже если в сообщении есть эмодзи или смайлики:\n"
        "     «Ваш запрос принят и передан владельцу аккаунта. Он свяжется с вами при необходимости.»\n"
        "   • Если написали только приветствие («Привет», «Шо ты») без конкретики — вежливо попроси изложить суть делового вопроса в одном сообщении.\n\n"
        "2. СПАМ, ФЛУД, ПЕСНИ, МЕМЫ И ЧАТЫ:\n"
        "   На бессмысленный флуд, мемы, предложения вступить в каналы, беседы или игры отвечай ИСКЛЮЧИТЕЛЬНО:\n"
        "   «Пожалуйста, удалите этот чат и не тратьте наше время»\n\n"
        "3. ПРАВИЛА ТЕКСТА: Чистый текст БЕЗ Markdown (без звёздочек ** и решёток #). Кратко (1-2 предложения)."
    ),
    "blogger": (
        "Ты — официальный ИИ-менеджер по рекламе блогера.\n\n"
        "1. РЕКЛАМНЫЕ ЗАПРОСЫ (ВЫСШИЙ ПРИОРИТЕТ):\n"
        "   • Если нет деталей — запроси ссылку на продукт, формат интеграции, желаемую дату и бюджет списком через '• '.\n"
        "   • Если данные уже есть — подтверди получение заявки.\n"
        "2. На спам, мемы и пустой флуд отвечай: «Пожалуйста, удалите этот чат и не тратьте наше время».\n"
...
PRESETS_EN = {
    "strict": (
        "You are a strict and professional AI secretary.\n"
        "1. BUSINESS & ADS (HIGHEST PRIORITY): If the message mentions business, ads, purchase, or collaboration, ALWAYS accept it, even with emojis: 'Your request has been received and forwarded to the account owner.'\n"
        "2. SPAM: If pure flood/memes/invites, reply: 'Please delete this chat and do not waste our time.'\n"
        "3. Plain text without Markdown."
    ),
    "blogger": (
        "You are an AI Ads Manager.\n"
        "1. Ads: Ask for product link, format, date, budget with '• ' bullets.\n"
        "2. Spam: Reply 'Please delete this chat and do not waste our time.'\n"
        "3. Plain text without Markdown."
    ),
    "support": (
    ...
    "vacation": (
        "You are a polite AI secretary. Owner is offline.\n"
        "Plain text without Markdown."
    ),
    "consultant": (
...
TEXTS = {
    "ru": {
        ...
        "broadcast_title": "📢 <b>Рассылка сообщений</b>\n\nОтправьте текст или фото для всех пользователей:",
        "broadcast_empty": "❌ Нет зарегистрированных пользователей.",
        ...
        "about_text": (
            ...
            "🧠 <b>Долгосрочная память:</b> бот помнит твои интересы и проекты через сессии.\n"
            "⚡️ <b>Live Stream:</b> потоковая генерация в реальном времени.\n"
            "🖼 <b>Мультимодальность:</b> поддержка анализа изображений и стикеров."
        ),
        "btn_summary": "📋 Выжимка",
        "summary_loading": "⏳ <i>Анализирую диалог и составляю бриф...</i>",
        "summary_empty": "❌ В памяти пока нет сообщений из этого диалога.",
        ...
        "memory_view": "🧠 <b>Что я помню о вас:</b>\n\n{memory}\n\n<i>Память обновляется автоматически в ходе диалогов. Вы можете очистить её командой /forget</i>",
        "memory_empty": "<i>Пока фактов не накоплено. Расскажите мне о себе, своих проектах или увлечениях!</i>",
        "memory_cleared": "🧹 <b>Долгосрочная память полностью очищена!</b>",
        "chat_reset_msg": "✨ <b>Контекст текущего диалога сброшен!</b>\nНачинаем с чистого листа, но важные факты о вас сохранены.",
        "media_fallback": "Пожалуйста, изложите суть вашего вопроса текстом в одном сообщении.",
        "vision_not_supported": "⚠️ Текущая модель <b>{model}</b> является чисто текстовой и не умеет анализировать изображения.\n\n👉 Откройте <b>⚙️ Настройки</b> ➔ <b>🤖 Выбрать модели</b> ➔ <b>💬 Для общения в ЛС</b> и выберите модель с меткой <b>(Мультимод.)</b> — например, <b>Gemini 2.0 Flash</b> или <b>Gemini 1.5 Pro</b>."
    },
    "en": {
        "start": "👋 <b>Hello! I am your AI assistant with persistent long-term memory.</b>\n\n• Use the buttons below the keyboard for quick navigation.\n• Click <b>⚙️ Settings</b> to configure models.",
        "panel_title": "⚙️ <b>AI Secretary Control Panel</b> ({role})",
        "role_owner": "👑 Creator",
        "role_user": "👤 User",
        "cur_models": "📌 <b>Models:</b>\n• <b>Direct Messages:</b> {dm_model}\n• <b>Business Chats:</b> {biz_model}",
        "cur_preset": "🎭 <b>Secretary Mode:</b> {preset}",
        ...
        "custom_model_prompt": "✍️ <b>Enter exact OpenRouter Model ID for {target}:</b>",
        ...
        "memory_view": "🧠 <b>What I remember about you:</b>\n\n{memory}\n\n<i>Updated automatically. Use /forget to wipe.</i>",
        "memory_empty": "<i>No facts recorded yet. Tell me about yourself!</i>",
        "memory_cleared": "🧹 <b>Long-term memory cleared!</b>",
        "chat_reset_msg": "✨ <b>Topic context reset!</b> Starting fresh.",
        "media_fallback": "Please state the core of your inquiry in one text message.",
        "vision_not_supported": "⚠️ Current model <b>{model}</b> is text-only.\n\n👉 Go to <b>⚙️ Settings</b> ➔ <b>🤖 Select Models</b> ➔ <b>💬 Direct Messages</b> and pick a model labeled <b>(Мультимод.)</b>."
    }
}
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS user_memories 
                 (user_id INTEGER PRIMARY KEY, memory_text TEXT, updated_at INTEGER)''')
...
async def update_memory_in_background(user_id: int, user_text: str, current_memory: str):
    ...
    extraction_prompt = (
        ...
        "Твоя задача — извлекать долговечные факты о пользователе (имя, проекты, увлечения, предпочтения, стек технологий, стиль общения).\n"
        f"Текущая карточка памяти:\n{current_memory or 'Пока пусто'}\n\n"
        f"Новое сообщение пользователя:\n«{user_text}»\n\n"
        "ИНСТРУКЦИЯ:\n"
        "• Если в сообщении есть новая важная информация о пользователе — дополни карточку памяти.\n"
        "• Формат: маркированный список через '• '.\n"
        "• Если ничего важного о личности или проектах нет — просто верни текущую карточку памяти БЕЗ изменений.\n"
        "• Верни ТОЛЬКО итоговый список фактов."
    )
...
# 💬 МОДЕЛИ ДЛЯ ЛС (С включением мощной Gemini 1.5 Pro)
PUBLIC_DM_MODELS = [
...
# 💼 МОДЕЛИ ДЛЯ БИЗНЕС-ЧАТОВ (Экономичные и быстрые)
PUBLIC_BIZ_MODELS = [
    ("🔹 DeepSeek V3 🐳", "deepseek/deepseek-chat"),
    ("🔹 Gemini 2.0 Flash ⚡️ (Мультимод.)", "google/gemini-2.0-flash-001"),
    ("🔹 Llama 3.3 70B 🦙", "meta-llama/llama-3.3-70b-instruct"),
...
def update_user_biz_model(user_id: int, new_model: str):
    ...
    c.execute('''INSERT INTO user_configs (user_id, model, system_prompt, lang, preset, biz_model, show_disclaimer) 
                 VALUES (?, ?, ?, ?, ?, ?, ?) 
                 ON CONFLICT(user_id) DO UPDATE SET biz_model = excluded.biz_model''', 
...
def get_advanced_keyboard(is_owner: bool, lang: str, show_disclaimer: int = 1):
    ...
    if lang == "en":
        disc_status = "On ✅" if show_disclaimer == 1 else "Off ❌"
...
async def show_panel(message: types.Message, user_id: int):
    ...
    dm_pretty = get_pretty_model_name(dm_model)
    biz_pretty = get_pretty_model_name(biz_model)
    
    text = (
        f"{t('panel_title', lang, role=role_badge)}\n\n"
        f"{t('cur_models', lang, dm_model=dm_pretty, biz_model=biz_pretty)}\n"
...
async def cmd_memory(message: types.Message):
    ...
    _, _, lang, _, _, _ = get_user_config(message.from_user.id)
    text = t("memory_view", lang, memory=mem if mem else t("memory_empty", lang))
...
async def handle_callbacks(call: types.CallbackQuery, state: FSMContext):
    ...
    if data == "choose_model_target":
        ...
        await call.answer()
        target = data.split(":")[1]
        target_name = t("target_dm_name", lang) if target == "dm" else t("target_biz_name", lang)
        title = t("model_choose_title", lang, target=target_name)
        await call.message.edit_text(title, reply_markup=get_models_keyboard(target, is_owner, lang), parse_mode="HTML")
        ...
        await call.answer(t("preset_set", lang, name=p_name), show_alert=False)
        await show_panel(call.message, uid)
        ...
        await call.message.edit_text(
            t("panel_title", lang, role=t("role_owner", lang) if is_owner else t("role_user", lang)), 
            reply_markup=get_advanced_keyboard(is_owner, lang, show_disclaimer), 
            parse_mode="HTML"
        )
        ...
        if lang == "en":
            toast = "AI Watermark enabled" if new_val == 1 else "AI Watermark disabled"
        ...
        await call.message.edit_text(t("custom_model_prompt", lang, target=target_name), reply_markup=cancel_kb, parse_mode="HTML")
        ...
        summary_prompt = (
            "Ты — ИИ-аналитик лидов. Проанализируй диалог и сформируй карточку сделки в Telegram HTML:\n"
            "🎯 <b>Услуга / Задача:</b> [суть]\n"
            "💰 <b>Бюджет:</b> [сумма]\n"
            "⏳ <b>Сроки / Дедлайн:</b> [сроки]\n"
            "📌 <b>Ключевые детали:</b> [описание]"
        )
        ...
        await call.answer("🛑 Бот заглушен в этом чате на 24 часа", show_alert=False)
        ...
        await call.answer("▶️ Бот снова активен в этом чате", show_alert=False)
...
# ⚡️ ПОТОКОВЫЙ СТРИМИНГ С ПОДДЕРЖКОЙ VISION (ИЗОБРАЖЕНИЙ)
async def ask_openrouter_stream(target_message: types.Message, user_text: str, custom_system_prompt: str = None, model: str = None, lang: str = "ru", image_b64: str = None):
    ...
    if image_b64:
        user_msg_content = [
            {"type": "text", "text": clean_text or "Что изображено на этом фото?"},
...
async def handle_business(message: types.Message):
    ...
    try:
        ...
        if message.text:
            ...
            is_media_without_text = True
            user_text = "[GIF-анимация]"
        elif message.photo:
            is_media_without_text = True
            user_text = "[Фотография]"
        elif message.voice or message.video_note:
            is_media_without_text = True
            user_text = "[Голосовое/Видео]"
        elif message.document:
            is_media_without_text = True
            user_text = f"[Файл: {message.document.file_name or 'документ'}]"
        else:
            return
        ...
        if is_media_without_text:
            if was_dismissed:
                ...
                spam_reply = spam_base + disclaimer_suffix
                try:
                    await message.answer(spam_reply)
                except Exception as e:
                    logging.error(f"Media repeat dismiss error: {e}")
            else:
                fallback_text = t("media_fallback", lang) + disclaimer_suffix
                try:
                    await message.answer(fallback_text)
                except Exception as e:
                    logging.error(f"Media fallback error: {e}")
            return
        ...
        if not show_disclaimer:
            ...
            ai_reply = re.sub(r'(?i)\n*This reply was generated by AI assistant.*', '', ai_reply).strip()
        else:
            if "сгенерирован ai" not in ai_reply.lower() and "generated by ai" not in ai_reply.lower():
        ...
        is_greeting_only = clean_text_check in GREETING_WORDS or len(clean_text_check) <= 3

        if not is_greeting_only or has_business_intent:
            ...
            try:
                kb = get_notify_keyboard(chat_id, is_paused=False, lang=lang)
...
async def handle_direct_sticker(message: types.Message):
    ...
    user_prompt = f"[Пользователь отправил стикер: {sticker_emoji}]"
    
    user_mem = get_user_memory(uid)
    pretty_name = get_pretty_model_name(dm_model)
    base_prompt_tmpl = DIRECT_PROMPTS.get(lang, DIRECT_PROMPTS["ru"])
    base_prompt = base_prompt_tmpl.format(model_name=pretty_name)
    if user_mem:
        base_prompt += f"\n\nПРОФИЛЬ И ВАЖНЫЕ ФАКТЫ О ПОЛЬЗОВАТЕЛЕ:\n{user_mem}"
        
    _, tokens_used = await ask_openrouter_stream(message, user_prompt, custom_system_prompt=base_prompt, model=dm_model, lang=lang)
    log_stat(uid, tokens=tokens_used, is_spam=False)

# 🖼 ОБРАБОТЧИК ФОТОГРАФИЙ В ЛС (VISION)
@dp.message(F.photo)
async def handle_direct_photo(message: types.Message):
    ...
    # Проверка, умеет ли модель работать с изображениями
    if not is_multimodal_model(dm_model):
        model_display = get_pretty_model_name(dm_model)
        await message.answer(t("vision_not_supported", lang, model=model_display), parse_mode="HTML")
        return
        
    status_indicator = await message.answer("⏳ <i>Загружаю и анализирую изображение...</i>", parse_mode="HTML")
    try:
        ...
        _, tokens_used = await ask_openrouter_stream(
            message, 
            caption, 
        ...
        logging.error(f"Direct photo handling error: {e}")
        try:
            await status_indicator.edit_text(t("ai_error", lang))
        except Exception:
            pass
...
async def handle_direct_text(message: types.Message, state: FSMContext):
    ...
    if text in ("⚙️ Настройки", "⚙️ Settings"):
        ...
        stats_text = t("stats_title", lang, msg_count=msg_count, tokens=tokens, spam_count=spam_count, recent_leads=leads_str)
    ...
    _, tokens_used = await ask_openrouter_stream(message, message.text, custom_system_prompt=base_prompt, model=dm_model, lang=lang)
    log_stat(uid, tokens=tokens_used, is_spam=False)
...
async def setup_bot_commands():
    commands_ru = [
        ...
        BotCommand(command="settings", description="Панель настроек моделей"),
        BotCommand(command="memory", description="Посмотреть память обо мне"),
        BotCommand(command="forget", description="Очистить память ассистента"),
    ]
    try:
