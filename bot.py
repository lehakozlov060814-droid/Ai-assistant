...
def init_db():
    ...
    # Таблицы подписок, команд и цен в Stars
    ...
    # Сидирование тарифов со звездами
...
# 💎 ЕДИНАЯ ВИТРИНА ТАРИФОВ И ПОДПИСОК БЕЗ ЛИШНИХ ЭКРАНОВ
async def show_subscriptions_storefront(message: types.Message, user_id: int):
    ...
    if sub_data:
        ...
        role_label = " <i>(вы в команде)</i>" if owner_id != user_id else ""
        text += (
...
async def fsm_do_assign(call: types.CallbackQuery, state: FSMContext):
    ...
    if target_uid:
        ...
        await call.answer("✅ Тариф выдан!", show_alert=True)
        try:
            await bot.send_message(target_uid, "🎉 <b>Вам подключен новый тариф!</b> Проверьте статус в <b>💎 Подписки</b>.", parse_mode="HTML")
        except Exception:
            pass
    await state.clear()
    await show_subscriptions_storefront(call.message, call.from_user.id)
