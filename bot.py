...
# 🏷 Расширенная база красивых названий моделей
MODEL_PRETTY_NAMES = {
...
def get_pretty_model_name(model_id: str) -> str:
    ...
    for key, val in MODEL_PRETTY_NAMES.items():
        if clean_id == key.lower():
            return val
    # Если модель кастомная или не найдена в словаре:
    if "/" in clean_id:
...
class FormStates(StatesGroup):
    ...
    waiting_for_memory_edit = State()

DIRECT_PROMPTS = {
    "ru": (
        "Ты — персональный умный Telegram-бот «Ai Assistant» с долгосрочной памятью.\n"
        "Ты помнишь пользователя, его интересы и проекты между разными сессиями и темами общения.\n\n"
        ...
        "2. В бизнес-чатах: умный авто-секретарь для Telegram Business (отвечает клиентам, отсекает спам, собирает лиды).\n\n"
        "КОМАНДЫ ДЛЯ ПОЛЬЗОВАТЕЛЯ:\n"
        "• /new или /reset — начать новый диалог (краткосрочная память очистится, но факты о пользователе сохранятся)\n"
        "• /memory — посмотреть профиль фактов\n"
        "• /forget — полностью стереть память о себе\n"
        "• /settings — панель моделей и пресетов секретаря\n\n"
        "ЕСЛИ СПРАШИВАЮТ ПРО ПОДКЛЮЧЕНИЕ СЕКРЕТАРЯ:\n"
        "Инструкция: Настройки Telegram ➔ «Telegram для бизнеса» ➔ «Чат-боты» ➔ добавить этого бота ➔ в «Чаты» выбрать «Все, кроме контактов»."
    ),
    "en": (
        "You are 'Ai Assistant', a smart personal Telegram bot with long-term cross-session memory.\n\n"
        "YOUR ROLE & CAPABILITIES:\n"
        "1. In direct messages: Personal assistant remembering key user facts across different topics.\n"
        "2. In business chats: Automated AI secretary for Telegram Business.\n\n"
        "USER COMMANDS:\n"
        "• /new or /reset — Start fresh topic (retains long-term memory facts)\n"
        "• /memory — View remembered profile facts\n"
        "• /forget — Wipe all stored user facts\n"
...
TEXTS = {
    "ru": {
        ...
        "cur_model": "📌 <b>Модель:</b> <b>{model}</b>",
        ...
        "model_set": "✅ Модель изменена на: <b>{model}</b>",
        ...
        "about_text": (
            ...
            "🧠 <b>Долгосрочная память:</b> бот запоминает твои интересы и проекты через сессии.\n"
            "⚡️ <b>Live Stream:</b> печать ответов в реальном времени.\n"
            "💼 <b>Telegram Business:</b> секретарь с Human Takeover на 3ч и антиспамом."
        ),
        "btn_summary": "📋 Выжимка",
        "btn_menu_new": "⚙️ Меню",
    ...
    "en": {
        ...
        "model_set": "✅ Model set to: <b>{model}</b>",
...
async def update_memory_in_background(user_id: int, user_text: str, current_memory: str, model: str):
    ...
    extraction_prompt = (
        ...
        "ИНСТРУКЦИЯ:\n"
        "• Если в сообщении есть новая важная информация о пользователе или изменения — дополни/обнови карточку памяти.\n"
        "• Формат: маркированный список через '• '.\n"
        "• Если ничего важного о личности или проектах пользователя нет — просто верни текущую карточку памяти БЕЗ изменений.\n"
        "• Не пиши никаких пояснений, верни ТОЛЬКО итоговый список фактов."
    )
...
async def show_panel(message: types.Message, user_id: int):
    ...
    display_model_name = get_pretty_model_name(model)
    
    text = (
        f"{t('panel_title', lang, role=role_badge)}\n\n"
        f"{t('cur_model', lang, model=display_model_name)}\n"
...
async def handle_callbacks(call: types.CallbackQuery, state: FSMContext):
    ...
    if data == "choose_model":
        ...
        await call.message.answer(t("model_set", lang, model=get_pretty_model_name(new_model)), parse_mode="HTML")
        ...
        summary_prompt = (
            "Ты — ИИ-аналитик лидов. Проанализируй диалог и сформируй карточку сделки в Telegram HTML (без markdown):\n"
...
async def handle_direct_text(message: types.Message, state: FSMContext):
    ...
    model, _, lang, _ = get_user_config(uid)
    
    user_mem = get_user_memory(uid)
    base_prompt = DIRECT_PROMPTS.get(lang, DIRECT_PROMPTS["ru"])
    if user_mem:
        base_prompt += f"\n\nПРОФИЛЬ И ВАЖНЫЕ ФАКТЫ О ПОЛЬЗОВАТЕЛЕ:\n{user_mem}"
    
    _, tokens_used = await ask_openrouter_stream(message, message.text, custom_system_prompt=base_prompt, model=model, lang=lang)
    log_stat(uid, tokens=tokens_used, is_spam=False)
    
    asyncio.create_task(update_memory_in_background(uid, message.text, user_mem, model))
