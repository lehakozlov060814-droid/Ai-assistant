...
TEXTS = {
    "ru": {
        ...
        "about_text": (
            ...
            "«Ai Assistant» — интеллектуальный помощник с долговременной памятью и персональный авто-секретарь для <b>Telegram Business</b>.\n\n"
            "💬 <b>Возможности в ЛС:</b>\n"
            "• <b>Live Stream:</b> потоковая печать ответов в реальном времени.\n"
            "• 🧠 <b>Долгосрочная память:</b> ассистент запоминает факты и проекты (просмотр: /memory, очистка: /forget).\n"
            "• 🖼 <b>Мультимодальность:</b> анализ фотографий (модели с меткой <i>Мультимод.</i>) и реакция на стикеры.\n"
            "• ✨ <b>Новый диалог (/new):</b> сброс темы беседы без стирания накопленной памяти.\n\n"
            "💼 <b>Возможности для Telegram Business:</b>\n"
            "• 🎭 <b>Специализированные пресеты:</b> строгий секретарь, поддержка клиентов, менеджер рекламы блогера, консультант или свой prompt.\n"
            "• 🛡 <b>Антиспам и экономия токенов:</b> отсекает бессмысленный флуд и спам без траты баланса.\n"
            "• 📩 <b>Лидогенерация:</b> мгновенно присылает карточку обращения клиента с кнопкой <b>[📋 Выжимка]</b>.\n"
            "• 🛑 <b>Human Takeover:</b> если владелец сам ответил клиенту, бот засыпает в этом чате на 3 часа (или на 24ч по кнопке).\n"
            "• 🏷 <b>Подпись ИИ:</b> возможность включить или скрыть подпись ассистента в доп. настройках.\n\n"
            "💎 <b>Подписки и команды:</b>\n"
            "Поддержка общих командных тарифов (Enterprise) с несколькими рабочими местами и общим пулом токенов."
        ),
        "btn_summary": "📋 Выжимка",
        ...
        "sub_info_title": "💎 <b>Управление подпиской и тарифами</b>\n\n{status}",
        "btn_sub_catalog": "📋 Каталог тарифов",
        "btn_sub_team": "👥 Моя команда",
        "btn_admin_create_plan": "➕ Создать тариф",
        "btn_admin_assign_plan": "🎁 Выдать тариф юзеру",
        "btn_admin_list_plans": "📑 Список тарифов"
    },
    "en": {
        ...
        "sub_info_title": "💎 <b>Subscription & Plans</b>\n\n{status}",
        "btn_sub_catalog": "📋 Plans Catalog",
        "btn_sub_team": "👥 My Team",
        "btn_admin_create_plan": "➕ Create Plan",
        "btn_admin_assign_plan": "🎁 Assign Plan",
        "btn_admin_list_plans": "📑 List Plans"
    }
}
...
def init_db():
    ...
    # Сидирование базовых тарифов при первом запуске
    c.execute("SELECT COUNT(*) FROM subscription_plans")
    if c.fetchone()[0] == 0:
        c.execute('''INSERT INTO subscription_plans (name, token_limit, period_days, max_seats, price_desc)
                     VALUES 
                     ('Solo Starter', 50000, 7, 1, 'Бесплатно / Базовый лимит на неделю'),
                     ('Team Pro 🚀', 300000, 7, 3, '490 ₽/нед — 3 рабочих места'),
                     ('Enterprise 🏢', 1200000, 30, 15, '1 990 ₽/мес — до 15 человек команды')''')
...
# --- ФУНКЦИИ ПОДПИСОК И КОМАНД ---
def get_all_plans():
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute("SELECT id, name, token_limit, period_days, max_seats, price_desc FROM subscription_plans ORDER BY id ASC")
    rows = c.fetchall()
    conn.close()
    return rows

def create_new_plan(name: str, token_limit: int, period_days: int, max_seats: int, price_desc: str):
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute('''INSERT INTO subscription_plans (name, token_limit, period_days, max_seats, price_desc)
                 VALUES (?, ?, ?, ?, ?)''', (name, token_limit, period_days, max_seats, price_desc))
    conn.commit()
    conn.close()
...
def get_effective_subscription(user_id: int):
    ...
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute('''SELECT us.user_id, us.tokens_used, us.period_end, sp.id, sp.name, sp.token_limit, sp.period_days, sp.max_seats, sp.price_desc
...
def add_team_member(team_owner_id: int, member_id: int) -> bool:
    ...
    # 1 место занимает сам создатель команды
...
def check_and_consume_tokens(user_id: int, tokens: int) -> bool:
    ...
    if not sub_data:
        return True # Если подписка не задана, не блокируем
    ...
    # Периодический авто-сброс лимитов
    if now >= period_end:
...
def get_subs_menu_keyboard(is_owner: bool, lang: str, has_team_plan: bool):
    kb = [
        [InlineKeyboardButton(text=t("btn_sub_catalog", lang), callback_data="subs_catalog")]
    ]
    if has_team_plan:
        kb.append([InlineKeyboardButton(text=t("btn_sub_team", lang), callback_data="subs_my_team")])
    if is_owner:
        kb.append([InlineKeyboardButton(text=t("btn_admin_create_plan", lang), callback_data="admin_create_plan")])
...
async def show_subscriptions_panel(message: types.Message, user_id: int):
    is_owner = (user_id == OWNER_ID)
    ...
    if sub_data:
        ...
        time_left_days = max(0, (period_end - int(time.time())) // 86400)
        
        has_team_plan = (max_seats > 1 and owner_id == user_id)
        role_note = "<i>(вы участник корпоративной команды)</i>" if owner_id != user_id else ""
        
        status = (
            f"📦 <b>Текущий тариф:</b> {plan_name} {role_note}\n"
            f"📊 <b>Использовано:</b> {tokens_used:,} / {token_limit:,} токенов\n"
            f"👥 <b>Мест в тарифе:</b> {max_seats}\n"
            f"⏳ <b>Сброс лимита через:</b> {time_left_days} дн.\n"
            f"ℹ️ <i>{price_desc}</i>"
        )
    else:
        status = "<i>У вас активен базовый бесплатный тариф. Выберите расширенный план в каталоге ниже!</i>"
        
    text = t("sub_info_title", lang, status=status)
    kb = get_subs_menu_keyboard(is_owner, lang, has_team_plan)
    try:
...
async def handle_callbacks(call: types.CallbackQuery, state: FSMContext):
    ...
    if data == "choose_model_target":
        ...
        await call.answer()
        await show_subscriptions_panel(call.message, uid)
    elif data == "subs_catalog":
        await call.answer()
        plans = get_all_plans()
        text = "📋 <b>Каталог доступных тарифов:</b>\n\n"
        for p in plans:
            text += f"🔹 <b>{p[1]}</b>\n• 🪙 Лимит токенов: {p[2]:,}\n• ⏳ Период сброса: {p[3]} дн.\n• 👥 Мест в тарифе: {p[4]}\n• 💳 Стоимость: <i>{p[5]}</i>\n\n"
        text += "<i>Для подключения тарифа или корпоративной интеграции свяжитесь с создателем бота.</i>"
        back_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=t("btn_back", lang), callback_data="view_subs")]])
        ...
        await call.message.edit_text("✍️ <b>Введите Telegram User ID сотрудника для добавления в команду:</b>\n<i>(Сотрудник должен предварительно нажать /start в этом боте)</i>", reply_markup=cancel_kb, parse_mode="HTML")
        ...
        await call.message.edit_text("1️⃣ <b>Введите название нового тарифа</b> (например: Enterprise Ultra):", reply_markup=cancel_kb, parse_mode="HTML")
        ...
        if is_owner:
            stats_text = t("stats_title_owner", lang, msg_count=msg_count, tokens=tokens, spam_count=spam_count, recent_leads=leads_str)
        else:
            stats_text = t("stats_title_user", lang, msg_count=msg_count, spam_count=spam_count, recent_leads=leads_str)
        ...
        summary_prompt = (
            ...
            "СТРОЖАЙШИЕ ПРАВИЛА:\n"
            "1. Начинай ответ СРАЗУ с символа 🎯 без вступительных фраз.\n"
            "2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать вступления («Вот карточка:»), заключения и инструкции в конце.\n"
            "3. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО оборачивать текст в блоки кода (никаких ``` или ```html).\n\n"
            "ШАБЛОН:\n"
            "🎯 <b>Суть вопроса:</b> [краткое описание проблемы или услуги]\n"
            "💰 <b>Бюджет:</b> [сумма / Бесплатно / Не указан]\n"
            "⏳ <b>Сроки:</b> [дедлайн или Не указаны]\n"
            "📌 <b>Ключевые детали:</b> [1-2 предложения с сутью]"
        )
        ...
        # Полная очистка от markdown-блоков и вступительного/заключительного мусора
...
async def fsm_plan_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("2️⃣ <b>Укажите лимит токенов</b> (числом, например 500000):")
...
async def fsm_plan_tokens(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("3️⃣ <b>Укажите период сброса лимита в днях</b> (числом, например 7 или 30):")
    except ValueError:
...
async def fsm_plan_period(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("4️⃣ <b>Укажите количество мест в тарифе</b> (1 для Solo, от 2 до 20 для Enterprise команд):")
    except ValueError:
        await message.answer("❌ Введите целое число:")
...
async def fsm_plan_seats(message: types.Message, state: FSMContext):
    try:
        ...
        await state.set_state(FormStates.create_plan_price)
        await message.answer("5️⃣ <b>Укажите описание стоимости</b> (например: 990 ₽/мес или Бесплатно):")
    except ValueError:
        await message.answer("❌ Введите целое число:")

@dp.message(FormStates.create_plan_price)
async def fsm_plan_price(message: types.Message, state: FSMContext):
    data = await state.get_data()
    create_new_plan(data["name"], data["tokens"], data["period"], data["seats"], message.text.strip())
    await state.clear()
    await message.answer(f"✅ <b>Тариф «{data['name']}» успешно создан и добавлен в каталог!</b>", parse_mode="HTML")
    await show_subscriptions_panel(message, message.from_user.id)
...
async def fsm_assign_user(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("❌ Введите корректный числовой User ID:")

@dp.callback_query(F.data.startswith("do_assign:"))
async def fsm_do_assign(call: types.CallbackQuery, state: FSMContext):
    plan_id = int(call.data.split(":")[1])
    data = await state.get_data()
    target_uid = data.get("target_uid")
    if target_uid:
        assign_plan_to_user(target_uid, plan_id)
        await call.answer("✅ Тариф успешно выдан!", show_alert=True)
        try:
            await bot.send_message(target_uid, "🎉 <b>Вам подключен новый тариф подписки!</b> Проверьте статус в разделе <b>💎 Подписки</b>.", parse_mode="HTML")
        except Exception:
            pass
    await state.clear()
    await show_subscriptions_panel(call.message, call.from_user.id)
...
async def fsm_add_team_member(message: types.Message, state: FSMContext):
    try:
        ...
        if success:
            ...
            try:
                await bot.send_message(mem_id, "🎉 <b>Вас добавили в корпоративную команду!</b> Теперь вы используете общий пул токенов тарифа.", parse_mode="HTML")
            except Exception:
                pass
        else:
            await message.answer("❌ <b>Не удалось добавить сотрудника:</b> лимит мест в вашем тарифе исчерпан.")
    except ValueError:
...
async def handle_direct_text(message: types.Message, state: FSMContext):
    ...
    if text in ("⚙️ Настройки", "⚙️ Settings"):
        ...
        await show_subscriptions_panel(message, uid)
        return
