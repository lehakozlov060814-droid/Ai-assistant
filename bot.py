...
DIRECT_PROMPTS = {
    "ru": (
        ...
        "Твоя текущая языковая модель: {model_name}. Если тебя спрашивают, какая ты модель или на чём работаешь — честно называй именно {model_name} (ты НЕ GPT-4, если сейчас выбран не он)!\n\n"
        ...
        "• /new или /reset — начать новый диалог (сбросить текущую тему без потери фактов)\n"
...
PRESETS_RU = {
    "strict": (
        ...
        "КРИТИЧЕСКИЕ ПРАВИЛА:\n"
        "1. СПАМ, ФЛУД, ПЕСНИ, МЕМЫ И ЧАТЫ:\n"
        "   На любые предложения вступить в группы, беседы, игры или флуд отвечай ИСКЛЮЧИТЕЛЬНО:\n"
        "   «Пожалуйста, удалите этот чат и не тратьте наше время»\n\n"
        "2. ДЕЛОВЫЕ ЗАПРОСЫ:\n"
        "   • Если описана суть рабочей задачи/рекламы — подтверди приём: «Ваш запрос принят и передан владельцу аккаунта. Он свяжется с вами при необходимости.»\n"
        "   • Если написали только приветствие («Привет», «Шо ты») без темы — попроси изложить суть делового вопроса в одном сообщении.\n\n"
        "3. Чистый текст БЕЗ Markdown. Кратко (1-2 предложения).\n"
        f"4. ВСЕГДА в конце на новой строке:\nЭтот ответ сгенерирован AI ассистентом 🤖 Ai Assistant [{BOT_VERSION}]"
    ),
    "blogger": (
    ...
    "support": (
        ...
        f"3. ВСЕГДА в конце на новой строке:\nЭтот ответ сгенерирован AI ассистентом 🤖 Ai Assistant [{BOT_VERSION}]"
    ),
    "vacation": (
...
PRESETS_EN = {
    "strict": (
        "You are a strict AI secretary.\n"
        "1. SPAM: Reply 'Please delete this chat and do not waste our time.'\n"
        "2. BUSINESS: Acknowledge or ask details in one message.\n"
        f"3. Append: This reply was generated by AI assistant 🤖 Ai Assistant [{BOT_VERSION}]"
    ),
    "blogger": (
        "You are an AI Ads Manager.\n"
        "1. Spam: Reply 'Please delete this chat and do not waste our time.'\n"
        "2. Ask specs with '• ' bullets.\n"
        f"3. Append: This reply was generated by AI assistant 🤖 Ai Assistant [{BOT_VERSION}]"
    ),
    "support": (
        "You are a Support AI. Collect issue details with bullets.\n"
        f"Append: This reply was generated by AI assistant 🤖 Ai Assistant [{BOT_VERSION}]"
    ),
    "vacation": (
...
TEXTS = {
    "ru": {
        "start": "👋 <b>Привет! Я твой персональный ИИ-ассистент с долгосрочной памятью.</b>\n\n• <b>/new</b> — начать новый диалог (память о вас сохранится)\n• <b>/memory</b> — профиль ваших интересов и фактов\n• <b>/settings</b> — настройки и подключение к Telegram Business",
        ...
        "model_set": "✅ Модель для {target} изменена на: <b>{model}</b>",
        "custom_model_prompt": "✍️ <b>Введите точный ID модели с OpenRouter для {target}:</b>",
        "custom_model_set": "✅ Модель для {target} установлена: <code>{model}</code>",
    ...
    "en": {
        ...
        "model_set": "✅ Model for {target} set to: <b>{model}</b>",
        "custom_model_prompt": "✍️ <b>Enter exact OpenRouter Model ID for {target}:</b>",
        "custom_model_set": "✅ Model for {target} set to: <code>{model}</code>",
        ...
        "btn_clear": "🧹 Clear",
        "btn_block": "⛔️ Block",
        "muted_msg": "🛑 Bot is muted for <b>24 hours</b>.",
        "unmuted_msg": "▶️ Unmuted!",
        "cleared_msg": "🧹 Memory cleared!",
        "blocked_msg": "⛔️ Added to <b>Blacklist</b>.",
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS user_configs 
                 (user_id INTEGER PRIMARY KEY, model TEXT, system_prompt TEXT, lang TEXT, preset TEXT, biz_model TEXT)''')
...
async def update_memory_in_background(user_id: int, user_text: str, current_memory: str):
    if len(user_text.strip()) < 12 or user_text.startswith("/"):
        return
...
def get_user_config(user_id: int):
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute("SELECT model, system_prompt, lang, preset, biz_model FROM user_configs WHERE user_id = ?", (user_id,))
    row = c.fetchone()
    conn.close()
    if row:
        ...
        return dm_model, prompt, lang, preset, biz_model
    return "deepseek/deepseek-chat", PRESETS_RU["strict"], "ru", "strict", "deepseek/deepseek-chat"

def update_user_dm_model(user_id: int, new_model: str):
    _, prompt, lang, preset, biz_model = get_user_config(user_id)
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute('''INSERT INTO user_configs (user_id, model, system_prompt, lang, preset, biz_model) 
                 VALUES (?, ?, ?, ?, ?, ?) 
                 ON CONFLICT(user_id) DO UPDATE SET model = excluded.model''', 
              (user_id, new_model, prompt, lang, preset, biz_model))
    conn.commit()
    conn.close()

def update_user_biz_model(user_id: int, new_model: str):
    dm_model, prompt, lang, preset, _ = get_user_config(user_id)
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute('''INSERT INTO user_configs (user_id, model, system_prompt, lang, preset, biz_model) 
                 VALUES (?, ?, ?, ?, ?, ?) 
                 ON CONFLICT(user_id) DO UPDATE SET biz_model = excluded.biz_model''', 
              (user_id, dm_model, prompt, lang, preset, new_model))
    conn.commit()
    conn.close()

def update_user_prompt(user_id: int, new_prompt: str):
    dm_model, _, lang, _, biz_model = get_user_config(user_id)
    conn = sqlite3.connect("bot_config.db")
    c = conn.cursor()
    c.execute('''INSERT INTO user_configs (user_id, model, system_prompt, lang, preset, biz_model) 
                 VALUES (?, ?, ?, ?, ?, ?) 
                 ON CONFLICT(user_id) DO UPDATE SET system_prompt = excluded.system_prompt, preset = excluded.preset''', 
              (user_id, dm_model, new_prompt, lang, "custom", biz_model))
    conn.commit()
    conn.close()
...
def update_user_preset(user_id: int, preset_key: str):
    ...
    c.execute('''INSERT INTO user_configs (user_id, model, system_prompt, lang, preset, biz_model) 
                 VALUES (?, ?, ?, ?, ?, ?) 
                 ON CONFLICT(user_id) DO UPDATE SET system_prompt = excluded.system_prompt, preset = excluded.preset''', 
              (user_id, dm_model, new_prompt, lang, preset_key, biz_model))
    conn.commit()
    conn.close()

def update_user_lang(user_id: int, new_lang: str):
    dm_model, prompt, _, preset, biz_model = get_user_config(user_id)
    ...
    c.execute('''INSERT INTO user_configs (user_id, model, system_prompt, lang, preset, biz_model) 
              ...
              (user_id, dm_model, prompt, new_lang, preset, biz_model))
    conn.commit()
    conn.close()
...
async def show_panel(message: types.Message, user_id: int):
    ...
    dm_model, _, lang, preset, biz_model = get_user_config(user_id)
...
async def cmd_start(message: types.Message):
    ...
    _, _, lang, _, _ = get_user_config(message.from_user.id)
...
async def cmd_reset_context(message: types.Message):
    ...
    _, _, lang, _, _ = get_user_config(message.from_user.id)
    await message.answer(t("chat_reset_msg", lang), parse_mode="HTML")
...
async def handle_callbacks(call: types.CallbackQuery, state: FSMContext):
    ...
    dm_model, prompt, lang, preset, biz_model = get_user_config(uid)
    data = call.data
    ...
    if data == "choose_model_target":
        ...
        await call.message.answer(t("model_set", lang, target=t_name, model=get_pretty_model_name(new_model)), parse_mode="HTML")
...
async def process_custom_model(message: types.Message, state: FSMContext):
    ...
    if message.from_user.id == OWNER_ID:
        if target == "dm":
            ...
            t_name = t("target_biz_name", lang)
        await message.answer(t("custom_model_set", lang, target=t_name, model=new_model), parse_mode="HTML")
    await state.clear()
    await show_panel(message, message.from_user.id)
...
async def handle_business(message: types.Message):
    ...
    try:
        ...
        _, system_prompt, lang, _, biz_model = get_user_config(current_owner)
        ...
        # Бизнес-секретарь отвечает своей настроенной моделью
...
async def handle_direct_text(message: types.Message, state: FSMContext):
    ...
    dm_model, _, lang, _, _ = get_user_config(uid)
    
    user_mem = get_user_memory(uid)
    pretty_name = get_pretty_model_name(dm_model)
    
    # Подставляем реальное название модели прямо в промпт для устранения галлюцинаций
    ...
    # В ЛС используется персональная модель dm_model
