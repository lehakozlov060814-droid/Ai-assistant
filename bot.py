...
DIRECT_PROMPTS = {
    "ru": (
        ...
        "• /memory — посмотреть или отредактировать то, что ты о нем помнишь\n"
        "• /forget — полностью стереть память о пользователе\n"
    ...
    "en": (
        ...
        "• /memory — View or manage remembered profile facts\n"
...
# 🧠 Асинхронное фоновое извлечение фактов (не тормозит отправку сообщения)
async def update_memory_in_background(user_id: int, user_text: str, current_memory: str, model: str):
    # Пропускаем служебные короткие фразы
...
# 🧹 КОМАНДА СБРОСА КОНТЕКСТА ТЕКУЩЕГО ДИАЛОГА
...
# 🧹 КОМАНДА ОЧИСТКИ ДОЛГОСРОЧНОЙ ПАМЯТИ
...
# ⚡️ ПЛАВНЫЙ СТРИМИНГ С ДОЛГОСРОЧНОЙ ПАМЯТЬЮ
...
async def handle_direct_text(message: types.Message, state: FSMContext):
    ...
    # 🧠 ПОДТЯГИВАЕМ ДОЛГОСРОЧНУЮ ПАМЯТЬ ПОЛЬЗОВАТЕЛЯ
    ...
    # 🧠 Фоновое обновление фактов в SQLite (не задерживает ответ пользователю)
