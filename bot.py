...
# Проверка и авто-сброс удалённых из системы моделей
...
async def ask_openrouter_stream(target_message: types.Message, user_text: str, custom_system_prompt: str = None, model: str = None, lang: str = "ru", image_b64: str = None):
        ...
        try:
            async with session.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=payload) as resp:
                if resp.status != 200:
                    ...
                    # Показываем детальный код создателю для мгновенной диагностики
    ...
    # Записываем пару в память только после успешного ответа
