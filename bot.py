...
PRESETS_RU = {
    "strict": (
        ...
        "2. ДЕЛОВЫЕ ЗАПРОСЫ:\n"
        "   • Если описана суть конкретной задачи или рекламы — подтверди приём: «Ваш запрос принят и передан владельцу аккаунта. Он свяжется с вами при необходимости.»\n"
        "   • Если написали только приветствие («Привет», «Шо ты») без темы — попроси изложить суть делового вопроса в одном сообщении.\n\n"
        "3. Чистый текст БЕЗ Markdown (без звёздочек ** и решёток #). Кратко (1-2 предложения)."
    ),
    "blogger": (
...
# 📩 ЛАКОНИЧНАЯ КЛАВИАТУРА ДЛЯ УВЕДОМЛЕНИЙ В ЛС (ТОЛЬКО 2 КНОПКИ В 1 РЯД)
...
async def handle_business(message: types.Message):
    ...
    try:
        ...
        # Медиа без подписи (стикеры, гифки) — быстрый ответ без вызова нейросети
        ...
        # Динамическое управление подписью ИИ
        ...
        # Фильтр уведомлений в ЛС владельца (не беспокоить на пустые приветствия)
