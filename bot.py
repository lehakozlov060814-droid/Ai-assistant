...
# ⌨️ ПОСТОЯННАЯ КЛАВИАТУРА ПОД ПОЛЕМ ВВОДА (REPLY KEYBOARD)
...
# 🛠 ПАНЕЛЬ НАСТРОЕК (БЕСШУМНЫЙ IN-PLACE РЕНДЕР)
...
# 🔄 ОБРАБОТКА ИНЛАЙН-КНОПОК (БЕЗ СООБЩЕНИЙ В ЧАТ)
@dp.callback_query()
async def handle_callbacks(call: types.CallbackQuery, state: FSMContext):
    ...
    if data == "choose_model_target":
        ...
        # Всплывающее уведомление сверху экрана вместо нового сообщения
...
# 💬 ОБРАБОТКА ТЕКСТА И КНОПОК НИЖНЕЙ КЛАВИАТУРЫ
...
async def handle_direct_text(message: types.Message, state: FSMContext):
    ...
    # Перехват кнопок постоянной нижней клавиатуры
...
# 📋 РЕГИСТРАЦИЯ КОМАНД ДЛЯ КНОПКИ «МЕНЮ» И СТРОКИ «/»
